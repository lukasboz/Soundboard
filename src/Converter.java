...
public final class Converter {
	...
	private void convert(InputStream input, OutputStream output, AudioFormat targetFormat) throws Exception {
		...
		try (final AudioInputStream rawSourceStream = AudioSystem.getAudioInputStream(input)) {
			...
			try (final AudioInputStream sourceStream = AudioSystem.getAudioInputStream(convertFormat, rawSourceStream);
	...
	private AudioFormat getAudioFormat(AudioFormat sourceFormat) {
		return new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, sourceFormat.getSampleRate(), 16,
	...
	public String toString() {
		return "Converter [input=" + input.toString() + ", audioFormat=" + audioFormat + ", close=" + close + "]";
	}
